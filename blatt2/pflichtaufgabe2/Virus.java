public class Virus {
    public static void main(String[] args) {
        //Eingaben in der Console auswerten
        ...
        tagNummer++;
        //Erste Wocheninzidenzberechnung
        ...
        //Visuelle Darstellung in der Konsole
        ...
        while (istJemandInfiziert) {
            ...
            for (int  i = 0; i < welt.length; i++) {
                for (int  j = 0; j < welt.length; j++) {
                    if (welt[i][j] > 1 && weltNeuinfiziert[i][j] == false) {
                        welt[i][j]--;
                        neuinfektionen = infizierungPlus(welt, weltNeuinfiziert,
                        i + 1, j, infektionProzent, neuinfektionen);
                        neuinfektionen = infizierungMinus(welt, weltNeuinfiziert,
                        i - 1, j, infektionProzent, neuinfektionen);
                        neuinfektionen = infizierungPlus(welt, weltNeuinfiziert,
                        i, j + 1, infektionProzent, neuinfektionen);
                        neuinfektionen = infizierungMinus(welt, weltNeuinfiziert,
                        i, j - 1, infektionProzent, neuinfektionen);
                    }
                }
            ...
            System.out.println("_____________________________________");
            //Check ob noch jemand infiziert ist
    ...
    */
    ...
    /**
    checkt ob uninfizierte neben dem infizierten sind, wenn ja, wird die
    wahrscheinlichkeit berechnet dass dieser angesteckt wird.
    @param welt
    @param weltNeuinfiziert
    @param i
    @param j
    @param infektionProzent
    @param neuinfektionen
    @return neuinfektionen
    */
    public static int infizierungPlus(int[][] welt, boolean[][] weltNeuinfiziert,
        int i, int j, float infektionProzent, int neuinfektionen) {
    ...
    /**
    checkt ob neben dem Infizierten Unizizierte sind, wenn ja steckt er
    diese mit der eingegebenen Wahrscheinlichkeit an.
    @param welt
    @param weltNeuinfiziert
    @param i
    @param j
    ...
    @return neuinfektionen
    */
    public static int infizierungMinus(int[][] welt, boolean[][] weltNeuinfiziert,
        int i, int j, float infektionProzent, int neuinfektionen) {
        if (j >=  0 && i >= 0) {
            if (welt[i][j] == 0) {
                double chanceInfizierung = (Math.random() * 100);
